...
async def client_sender(context: aiocoap.Context, uri: str, size: int, rate: int, duration: int, warmup: int) -> List[float]:
    ...
    async def _run_async(self, host, port, cipher, size, rate, duration, warmup, oscore_context_cfg):
        ...
        try:
            if cipher == 'oscore':
                ...
                secctx = aiocoap.oscore.SecurityContext(master_secret=key, master_salt=salt, sender_id=sender_id, recipient_id=recipient_id)
